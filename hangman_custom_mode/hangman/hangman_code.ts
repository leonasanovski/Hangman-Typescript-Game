...
let guessedLetters: string[] = []; // letters guessed so far
let MISSES = 0; // count wrong guesses

interface JsonToTripleObj {
    label: string;
    hint: string;
    image_hint: string;
}
//this function returns random triple
async function randomTriple(): Promise<JsonToTripleObj> {
...
async function loadWordTriples(): Promise<JsonToTripleObj[]> {
...
function drawWordLinesOnCanvas(ctx: CanvasRenderingContext2D, word: string, guesses: string[]) {
    const startX = 100;
    const baseY = 450;
...
function createAlphabetButtons(word: string, onCorrect: () => void, onWrong: () => void) {
    const container = document.getElementById("letter-buttons");
    if (!container) return;
    container.innerHTML = "";
    for (let i = 65; i <= 90; i++) {
...
function drawHangman(stage: number, random_guessing_object: JsonToTripleObj) {
...
function main_run_game():void{
    ...
    const info_btn = document.getElementById("info_btn") as HTMLElement;
    const info_div = document.getElementById("info_div") as HTMLElement;
